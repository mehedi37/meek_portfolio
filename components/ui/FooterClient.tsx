...
import { HiArrowUp, HiCode, HiMail } from "react-icons/hi";
 ...
 * Receives profile and social links from server wrapper
 */
export function FooterClient({ className = "", profile, socialLinks }: FooterClientProps) {
  const currentYear = new Date().getFullYear();
  const displayName = profile?.full_name || "Portfolio";
  const shortName = profile?.short_name || displayName.split(" ")[0];
          ...
          <div className="lg:col-span-2">
            ...
            <p className="text-[var(--muted-foreground)] max-w-md mb-8 leading-relaxed">
              {profile?.about_me?.slice(0, 150) ||
                "Building modern, performant web applications with a focus on user experience and clean code. Let's create something amazing together."}
              {profile?.about_me && profile.about_me.length > 150 && "..."}
            </p>

            <SocialLinks size="md" links={socialLinks} />
          </div>
